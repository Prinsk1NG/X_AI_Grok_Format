# -*- coding: utf-8 -*-
"""
grok_xai_xml_pipeline_sv.py  v8.0 (硅谷 100 人版：Grok Web UI 抓取 + xAI SDK XML 深度提纯)
...
def save_and_renew_session(context):
    ...
    try:
        ...
        headers = {"Authorization": f"token {PAT_FOR_SECRETS}", "Accept": "application/vnd.github.v3+json"}
...
def main():
    ...
    print(f"昨晚硅谷在聊啥 v8.0 (硅谷100人 Grok网页抓取 + xAI提纯 - {mode_str})", flush=True)
    print("=" * 60, flush=True)
    ...
    # 🚨 测试模式下，为了加速，只抓取名单的前 14 个人
    selected_accounts = ALL_ACCOUNTS[:14] if TEST_MODE else ALL_ACCOUNTS
